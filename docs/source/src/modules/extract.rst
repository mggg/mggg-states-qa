gdutils.extract
===============

Module Functions
----------------
extract.read_file
~~~~~~~~~~~~~~~~~
.. autofunction:: gdutils.extract.read_file


Class gdutils.extract.ExtractTable
----------------------------------
.. autoclass:: gdutils.extract.ExtractTable


Class Methods
~~~~~~~~~~~~~
extract.ExtractTable.__init__
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autofunction:: gdutils.extract.ExtractTable.__init__


Instance Methods
~~~~~~~~~~~~~~~~
extract.ExtractTable.extract
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autofunction:: gdutils.extract.ExtractTable.extract

extract.ExtractTable.extract_to_file
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autofunction:: gdutils.extract.ExtractTable.extract_to_file

extract.ExtractTable.list_columns
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autofunction:: gdutils.extract.ExtractTable.list_columns

extract.ExtractTable.list_values
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autofunction:: gdutils.extract.ExtractTable.list_values


